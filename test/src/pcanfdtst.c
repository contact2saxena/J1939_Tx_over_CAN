...
#incldue <native/sem.h>
#endif

#define RT
#define ONE_TASK_PER_DEVICE
#define XENO_USE_SEM_TO_SYNC
...
#ifdef XENOMAI
#ifdef XENO_USE_SEM_TO_SYNC
	lprintf(DEBUG, "signaling parent I'm exiting...\n");
	err = rt_sem_v(&tasks_sem);
	if (err)
		lprintf(ALWAYS, "failed to signal semaphore: err=%d\n", err);
#endif
...
static void run_application(void)
{
	...
	for (i = 0; i < pcan_device_count; ) {
		...
		//err = rt_sem_p(&tasks_sem, rt_timer_ns2ticks(1000000));
		...
		if (err) {
			lprintf(ALWAYS,
				"failed to wait for semaphore: err=%d\n", err);
			break;
		}
		...
		if (err) {
			lprintf(ALWAYS,
				"failed to signal semaphore: err=%d\n", err);
			break;
		}
	}
#endif
